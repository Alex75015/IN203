...
computeMandelbrotSet( int W, int H, int maxIter )
{
    ...
    if (rank == 0)
    {
		...
		int tag = 78;
		std::vector<int> pixels_loc[nbp-1];
		for(int i=0; i<nbp; ++i){
			MPI_Irecv(&pixels_loc[i], 1, MPI_INT, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD,&reqs[i]); //changer MPI_DOUBLE par MPI_INT
		}
		
		for ( int i = 0; i < floor(H/(nbp-1)); ++i ) {
			computeMandelbrotSetRow(W, H, maxIter, i, pixels.data() + W*(H-i-1) );
		}
		MPI_Waitall(nbp-1, reqs , &stats);
		//~ MPI_Gather (&sendbuf, H/nbp, sendtype, &recvbuf, recvcnt, recvtype, root, comm )
	}
	else
	{
		MPI_Status status;
		int tag = 78;
		int H_new = floor(H/(nbp-1)*rank);
		std::vector<int> pixels_loc(W*H_new);
		for ( int i = 0; i < H_new; ++i ) {
			computeMandelbrotSetRow(W, H, maxIter, i, pixels_loc.data() + W*(H_new-i-1) ); //Attention on parcours le tableau dans sa hauteur donc
		...
		MPI_Send(&pixels_loc, 1, MPI_INT, 0, tag, MPI_COMM_WORLD);
