...
computeMandelbrotSet( int W, int H, int maxIter )
{
    ...
    int H_loc = H/nbp; // H local pour chaque processus
    std::vector<int> pixels_loc(W*H_loc);
    
    MPI_Barrier (MPI_COMM_WORLD) ;
    // Tous l e s processus s ’ attendent à l a l i g n e suivante
	// a f i n de démarrer en même temps l a p a r t i e du code
	// dont on veut mesurer l e s performances en p a r a l l è l e
	...
	for ( int i = (H / nbp) * (nbp - 1 - rank); i < (H / nbp) * (nbp - rank); ++i ) {
		computeMandelbrotSetRow(W, H, maxIter, i, pixels_loc.data() + W*((H / nbp) * (nbp - rank)-i-1) ); //Attention on parcours le tableau dans sa hauteur donc
		//~ std::cout << "le pixel du proccessus rank " << rank << " vaut " << i << " sur " << H_loc-1 << std::endl;
	} // attention il faut bien mettre H_new ici
    ...
    std::cout << "Temps calcul processus " << rank << " mandelbrot : " << elapsed_seconds.count() 
              << std::endl;
    return pixels_loc;
}
...
int main(int argc, char *argv[] ) 
 { 
    ...
    const int W = 800; //de base 800
    const int H = 600; // de base 600
    // Normalement, pour un bon rendu, il faudrait le nombre d'itérations
    // ci--dessous :
    //const int maxIter = 16777216;
    const int maxIter = 65536*8; // *8 dans exo de base
    ...
    MPI_Gather(iters.data(), W*H_loc, MPI_INT, iters_gathered.data(), W*H_loc, MPI_INT, 0, MPI_COMM_WORLD);
    // on envoie tout dans iters_gathered
    
    if(rank == 0)
    {
		savePicture("mandelbrot_final.tga", W, H, iters_gathered, maxIter);
	}
