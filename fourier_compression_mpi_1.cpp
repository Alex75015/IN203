...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels )
{
    ...
    std::uint32_t ni_loc = ni/nbp;
    std::complex<double>* X = new std::complex<double>[ni_loc*nj];
    std::fill(X, X+ni_loc*nj, std::complex<double>(0.,0.));
    
    firstrow = (nbp-(rank+1)) * ni_loc;
    endrow = (nbp-rank) * ni_loc ; 
    
    for( std::uint32_t k1 = firstrow; k1 < endrow; ++k1 )
    {
        for (std::uint32_t k2 = 0; k2 < nj; ++k2)
        {
            for (std::uint32_t n2 = firstrow; n2 < endrow; ++n2 )
            {
...
int main(int nargs, char* argv[])
{
    ...
    {   
		...
		MPI_Gather( fourier_loc, ni_loc*nj, MPI_DOUBLE,
                fourier_glob, ni_loc*nj, MPI_INT,
                0, // root
                MPI_COMM_WORLD               
               );
    ...
    else 
    {
		...
		MPI_Gather( fourier_loc, ni_loc*nj, MPI_INT,
                NULL, ni_loc*nj, MPI_INT,
                0, // root
                MPI_COMM_WORLD               
               );
    }
