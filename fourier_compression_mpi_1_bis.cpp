...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels )
{
	...
	std::uint32_t firstrow, endrow;
	
    constexpr const double pi = 3.14159265358979324;
    std::uint32_t nj_loc = nj/nbp;
    std::complex<double>* X = new std::complex<double>[ni*nj_loc];
    std::fill(X, X+ni*nj_loc, std::complex<double>(0.,0.));
    
    firstrow = (nbp-(rank+1)) * nj_loc;
    endrow = (nbp-rank) * nj_loc ; 
    
    for( std::uint32_t k1 = 0; k1 < ni; ++k1 )
    {
        for (std::uint32_t k2 = firstrow; k2 < endrow; ++k2)
        {
            for (std::uint32_t n2 = 0; n2 < ni; ++n2 )
            {
                std::complex<double> exp2(std::cos(-2*pi*n2*k2/height), std::sin(-2*pi*n2*k2/height));
                for (std::uint32_t n1 = firstrow; n1 < endrow; ++n1 )
                {
                    std::complex<double> exp1(std::cos(-2*pi*n1*k1/nj), std::sin(-2*pi*n1*k1/nj));
                    X[k1*endrow+k2] += double(pixels[3*(n1+n2*endrow)])*exp1*exp2;
                }
            }
        }
    }
	...
	std::cout << "Temps encodage image par DFT processus " << rank << " : " << elapsed_seconds.count() << " secondes\n";
    return X;
}
...
sparseSpectralComposition compressSpectralComposition( std::uint32_t width, std::uint32_t height, const std::complex<double>* plainCoefs, double tauxCompression )
{
	...
	std::chrono::duration < double >elapsed_seconds = end - start;
	std::cout << "Temps sélection des p% plus gros coefficients " << elapsed_seconds.count() << " secondes\n";
...
unsigned char* inversePartialDiscretTransformFourier( sparseSpectralComposition const& sparse )
{
	...
	std::chrono::duration < double >elapsed_seconds = end - start;
	std::cout << "Temps reconstitution de l'image compressée " << elapsed_seconds.count() << " secondes\n";
    return x;
}
...
int main(int nargs, char* argv[])
{
    ...
    std::complex<double>* fourier_loc = discretTransformFourier( width, height, image );
    
 
    
	if (rank == 0)
    {   
		// uniquement alloue sur proc 0 pour economiser de la memoire

		std::complex<double>* fourier_glob = new std::complex<double>[ni*nj];
		MPI_Gather( fourier_loc, ni*nj_loc, MPI_C_DOUBLE_COMPLEX ,
                fourier_glob, ni*nj_loc, MPI_C_DOUBLE_COMPLEX ,
                0, // root
                MPI_COMM_WORLD               
               );
        
		std::cout << "Je suis bien là\n";
		
		auto sparse = compressSpectralComposition(width, height, fourier_glob, taux); // temps très petit comparé aux autre temps (cf Q1)
    // pas besoin de paralléliser cette partie
		...
		if(error) std::cerr << "decoder error " << error << ": " << lodepng_error_text(error) << std::endl;
		// seulement le processus 0 s'occupe de la reconstitution et de la sauvegarde de l’image
    ...
    else 
    {
		...
		MPI_Gather( fourier_loc, ni*nj_loc, MPI_C_DOUBLE_COMPLEX ,
                NULL, ni*nj_loc, MPI_C_DOUBLE_COMPLEX ,
                0, // root
                MPI_COMM_WORLD               
               );
    }
	...
	std::chrono::duration < double >elapsed_seconds = end - start;
	std::cout << "Temps total compression image processus " << rank << " : " << elapsed_seconds.count() << " secondes\n";
