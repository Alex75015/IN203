...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels )
{
	...
	std::cout << "Temps encodage image par DFT : " << elapsed_seconds.count() << " secondes\n";
    return X;
}
...
int main(int nargs, char* argv[])
{
    ...
    {   
		...
		MPI_Gather( fourier_loc, ni*nj_loc, MPI_INT,
                fourier_glob, ni*nj_loc, MPI_INT,
                0, // root
                MPI_COMM_WORLD               
               );
	...
	std::chrono::duration < double >elapsed_seconds = end - start;
	std::cout << "Temps total compression image : " << elapsed_seconds.count() << " secondes\n";
